...
In this section, we will got through an easy numerical example of the HHL algorithm.
...
Then, we will go through all 5 phases of the HHl alorithm. 
...
If we calculate the solution classicaly we will obtain the solution vector 
\begin{equation}
\vec{x} = \begin{pmatrix} \frac{3}{8}\\ \frac{9}{8}\\ \end{pmatrix}
\end{equation}

As we already discussed, in the end of the HHL algorithm don't solve for the whole solution vector $\vec{x}$, as we are only interested in some estimation value $\ket x M \ket x$.
...
To cacluate the probability ratio, we have to devide by the square of each entry of $\vec{x}$.
...
Normaly, the QPE would calculate these numbers, but as the main focus of this paper is the HHL algorithm we will not go through each step of the QPE.
...
Additionaly, we also have to determine the eigenvalues. The eigenvalues $\lambda_j$ of the matrix $A$ are
\begin{equation}
\lambda_0 = \frac{2}{3}
\end{equation}
...
\end{equation}
as all registers are intialized with the zero state.
...
\subsubsection{Inversion of the eigenvalues}
    ...
    This is achieved by rotating the rotating the a-registers by the eigenvalues in the c-register.
...
\subsection{Inverse Quantum Phase Estimation}
    We now backtrack all the steps in the QPE trough IQPE.
