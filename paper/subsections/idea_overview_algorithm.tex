...
Firstly, we will specify the problem statement. 
After that, we will describe a short mathematical explanation. 
Then, we will take a look at the quantum circuit, explaining all the phases of the circuit.
...
We now have all the tools to solve the equation, with the help of the Kronecker delta $\delta_{ij}$, by inserting the definition of $A^{-1}$ and $\ket b$ into our original equation,
\begin{equation}
\begin{split}
\ket{x} &= A^{-1} \ket{b}\\
...
First, we will look at the sets of qubits needed for the algorithm. 
...
Afterward, we perform the rotation and invert the eigenvalues through rotary operations. 
...
We will need to uncompute the state, meaining we have to undo all operations up until now to unentangle the states, whilst keeping the inverted eigenvalues.
